...
class UploadHandler {
	...
	function __construct($folderID, $options = null, $initialize = true, $error_messages = null) {
		$this->response = array();
		$this->options = array(
			'script_url' => $this->get_full_url() . '/' . basename($this->get_server_var('SCRIPT_NAME')) . '?folderID=' . $folderID . '&',
			'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/' . $folderID . '/',
			'upload_url' => $this->get_full_url() . '/' . $folderID . '/',
			'user_dirs' => false,
			'mkdir_mode' => 0755,
			'param_name' => 'files',
...
//            'accept_file_types' => '/.+$/i',
			...
			'max_number_of_files' => 5, //changed by ruwan
